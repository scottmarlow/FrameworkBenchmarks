...
def start(args, logfile, errfile):
  ...
  try:
    ...
    subprocess.Popen("target/wildfly-8.0.0.Final/bin/standalone.sh -b 0.0.0.0", shell=True, env=wildfly_env, cwd="ee7-wildfly", stderr=errfile, stdout=logfile)
    return 0
  except subprocess.CalledProcessError:
    return 1
def stop(logfile, errfile):
  try:
    subprocess.check_call("target/wildfly-8.0.0.Final/bin/jboss-cli.sh --connect --command=:shutdown", shell=True, cwd="ee7-wildfly", stderr=errfile, stdout=logfile)
    return 0
  except subprocess.CalledProcessError:
    return 1
